 ...
 * of nodes with in a graph visualization. It also shows: - Decorators for edges
 * - Decorators for nodes - Decorators for aggregates (only on mouseOver) -
 * using a label-field which contains the String to be drawn by the decorators -
 * currently, only one Renderer (a LabelRenderer) ist used for all the decorator
 * groups (so all decorators show a text) - using different text formatting for
 * each decorator group - using any shape for the nodes (ShapeRenderer on the
 * nodes) together with a label (LabelRenderer on the nodes decorators) - all
 * decorator groups use the same Layout, they are centered on the item they
 * decorate
 *
...
public class AggregateDecoratorDemo extends Display {
    ...
    public AggregateDecoratorDemo(List<String> eNode, int[][] relation) {
        // initialize display and data
        super(new Visualization());
        initDataGroups(eNode, relation);
        // set up the renderers
        // draw the nodes as basic shapes
        Renderer nodeR = new ShapeRenderer(25);
        // draw aggregates as polygons with curved edges
        Renderer polyR = new PolygonRenderer(Constants.POLY_TYPE_LINE);//POLY_TYPE_CURVE
        ((PolygonRenderer) polyR).setCurveSlack(0.15f);
        DefaultRendererFactory drf = new DefaultRendererFactory();
        drf.setDefaultRenderer(nodeR);
        drf.add("ingroup('aggregates')", polyR);
        drf.add(new InGroupPredicate(EDGE_DECORATORS ), new LabelRenderer(VisualItem.LABEL)); //EDGE_DECORATORS
        ...
        // for the aggregates
        DECORATOR_SCHEMA.setDefault(VisualItem.TEXTCOLOR, ColorLib.gray(150));
    ...
    private void initDataGroups(List<String> eNode, int[][] relation) {
        // create sample graph
        ...
        for (int i = 0; i < eNode.size(); i++) {
            ...
            tmpArr = Arrays.asList(eNode.get(i).split("#"));
            g.getNode(i).setString(VisualItem.LABEL, tmpArr.get(1));
        }
        //g.getEdge(0).setString(VisualItem.LABEL, ""+1);
        if (eNode.size() > 1) {
            for (int i = 0; i < eNode.size(); i++) {
                for (int j = 0; j < eNode.size(); j++) {
                    if (relation[i][j] != 0) {
                        g.addEdge(g.getNode(i), g.getNode(j));
                        if (i == j) {
                            //g.getEdge(g.getNode(i), g.getNode(j)).setString(VisualItem.LABEL, String.valueOf(relation[i][j]));
                        }
                        ...
                        if (i != j) {
                            if ((relation[i][j] - 1) == 0) {
                                ...
                                g.getEdge(g.getNode(i), g.getNode(j)).setString(VisualItem.LABEL, String.valueOf(relation[i][j] - 1));
                            }
